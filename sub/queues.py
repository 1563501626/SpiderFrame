...
class RabbitMq:
    def __init__(self, engine):
        self.engine = engine
        self.priority_queue = self.engine.priority_queue
    ...
    @staticmethod
    def publish(channel, request, queue, priority=1):
        """发布"""
        channel.basic_publish(
            exchange='',
            routing_key=queue,
            body=request,
            properties=pika.BasicProperties(delivery_mode=2, priority=priority)  # 消息持久化
        )
    ...
    def consume(self, channel, queue, callback, prefetch_count):
        """消费"""
        if self.priority_queue:
            channel.queue_declare(queue=queue, durable=True, arguments={"x-max-priority": 10})  # 队列持久化
        else:
    ...
    @staticmethod
    def is_empty(queue_name=None):
        rabbitmq_host = config.mq_host
        rabbitmq_user = config.mq_user
        rabbitmq_pwd = config.mq_pwd
        rabbitmq_port = 15672
        res = requests.get(
            url='http://{}:{}/api/queues/{}'.format(rabbitmq_host, rabbitmq_port, "%2F"),
