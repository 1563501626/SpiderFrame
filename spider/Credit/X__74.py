...
class X74Spider(manager.Spider):
    ...
    def __init__(self, pagecount=50, *args, **kwargs):
        ...
        self.post_data = ezfuc.struct_data(
            "evaluation.entType=2&evalpage=2&evaluation.entName=&evaluation.dfStart=&evaluation.dfend=")
    ...
    def parse(self, response):
        ...
        _, total_pages = ezfuc.parse_html(total_pages, ["re", "'(.*?)'"])
    ...
    def deal_parse(self, response):
        ...
        for tr in trs:
            ...
            _, res = ezfuc.parse_html(onclick.replace("'", ""), ['re', "\((.*?)\)"])
            ...
            item_loader['六十日排名'] = ""
            item_loader['评价年度'] = format_time.y_m_d
            item_loader['网站维护代码'] = "X--74"
            item_loader['发布日期'] = ""
            item_loader['有效期'] = ""
            item_loader['省'] = "广东"
            item_loader['市'] = "珠海"
